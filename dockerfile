...
RUN ["/bin/sh"," -c set -eux; apt-get update; apt-get install -y --no-install-recommends ca-certificates p11-kit; rm -rf /var/lib/apt/lists/*; apt-get install -y git; apt-get install -y maven "]

#RUN ["/bin/sh"," git clone https://github.com/BrianDelmaire/ProjectAstronomy.git; cd /ProjectAstronomy; mvn package "]
#RUN ["/bin/sh"," mvn package "]

COPY SimpleAstronomyLib-0.3.0.jar /app
...
RUN ["/bin/sh", "cp /app/SimpleAstronomyLib-0.3.0.jar /usr/local/tomcat/"]

RUN ["/bin/sh", "-c set -eux;apt-get update;xargs -rt apt-get install -y --no-install-recommends < "$TOMCAT_NATIVE_LIBDIR/.dependencies.txt";rm -rf /var/lib/apt/lists/"]
